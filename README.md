...
- The ORRO product line is published on PyPI through 0.2.2, while this repository
  packages the 0.2.3 release candidate. The post-release target state is: `orro`
  0.2.3 is published on PyPI. It becomes true only after `v0.2.3` is tagged and
  the Trusted-Publishing workflow completes. Until then, a normal
  `pip install orro` installs 0.2.2.
...
development dogfood can run the 0.2.3 source directly against pinned Depone and
witnessd checkouts.
...
The 0.2.3 package metadata declares `witnessd>=2.4.0,<3.0.0` while keeping Depone and
...
product line is published on PyPI through 0.2.2; this repository sources 0.2.3,
whose publication will be completed as a separate release step after the
`v0.2.3` tag triggers the
...
product line is published on PyPI through 0.2.2, and this repository prepares
source version 0.2.3 with a `witnessd>=2.4.0,<3.0.0` dependency. The wrapper contains no
