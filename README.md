...
- The ORRO product line is published on PyPI through 0.1.1, while this repository
  packages the 0.2.0 release candidate. The post-release target state is: `orro`
  0.2.0 is published on PyPI. It becomes true only after `v0.2.0` is tagged and
  the Trusted-Publishing workflow completes. Until then, a normal
  `pip install orro` installs 0.1.1.
...
Yes. The published ORRO product line is installable from PyPI at 0.1.1, and
development dogfood can run the 0.2.0 source directly against pinned Depone and
witnessd checkouts.
...
product line is published on PyPI through 0.1.1; this repository sources 0.2.0,
whose publication will be completed as a separate release step after the
`v0.2.0` tag triggers the
