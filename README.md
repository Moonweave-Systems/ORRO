...
- The ORRO product line is published on PyPI through 0.1.0, while this repository
  packages the 0.1.1 release candidate. The post-release target state is: `orro`
  0.1.1 is published on PyPI. It becomes true only after `v0.1.1` is tagged and
  the Trusted-Publishing workflow completes. Until then, a normal
  `pip install orro` installs 0.1.0.
...
Yes. The published ORRO product line is installable from PyPI at 0.1.0, and
development dogfood can run the 0.1.1 source directly against pinned Depone and
witnessd checkouts.
...
The 0.1.1 package metadata declares `witnessd>=2.3.2` while keeping Depone and
...
product line is published on PyPI through 0.1.0; this repository sources 0.1.1,
whose publication will be completed as a separate release step after the
`v0.1.1` tag triggers the
...
product line is published on PyPI through 0.1.0, and this repository prepares
source version 0.1.1 with a `witnessd>=2.3.2` dependency. The wrapper contains no
