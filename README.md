...
- The ORRO product line is published on PyPI through 0.2.4, while this repository
  packages the 0.2.20 release candidate. The post-release target state is: `orro`
  0.2.20 is published on PyPI. It becomes true only after `v0.2.20` is tagged and
...
development dogfood can run the 0.2.20 source directly against pinned Depone and
witnessd checkouts.
...
The 0.2.20 package metadata declares `witnessd>=2.4.0,<3.0.0` while keeping Depone and
...
product line is published on PyPI through 0.2.15; this repository sources 0.2.20,
whose publication will be completed as a separate release step after the
`v0.2.20` tag triggers the
...
source version 0.2.20 with a `witnessd>=2.4.0,<3.0.0` dependency. The wrapper contains no
