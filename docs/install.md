...
The ORRO product line is published on PyPI through 0.2.9, while this repository
packages 0.2.10. The post-release target state is: `orro` 0.2.10 is published on
PyPI. It becomes true only after `v0.2.10` is tagged and the Trusted-Publishing
workflow completes. The 0.2.10 package metadata declares `witnessd>=2.4.0,<3.0.0`, and
...
Until the `v0.2.10` workflow completes, that command installs `orro` 0.2.9.
Development dogfood may instead use the 0.2.10 source checkout and pinned engine
...
published on PyPI through 0.2.9, this repository prepares source 0.2.10 for the
