...
packages 0.2.2. The post-release target state is: `orro` 0.2.2 is published on
PyPI. It becomes true only after `v0.2.2` is tagged and the Trusted-Publishing
workflow completes. The 0.2.2 package metadata declares `witnessd>=2.4.0,<3.0.0`, and
...
Until the `v0.2.2` workflow completes, that command installs `orro` 0.2.1.
Development dogfood may instead use the 0.2.2 source checkout and pinned engine
...
published on PyPI through 0.2.1, this repository prepares source 0.2.2 for the
