...
packages 0.2.17. The post-release target state is: `orro` 0.2.17 is published on
PyPI. It becomes true only after `v0.2.17` is tagged and the Trusted-Publishing
workflow completes. The 0.2.17 package metadata declares `witnessd>=2.4.0,<3.0.0`, and
...
Until the `v0.2.17` workflow completes, that command installs `orro` 0.2.15.
Development dogfood may instead use the 0.2.17 source checkout and pinned engine
...
published on PyPI through 0.2.15, this repository prepares source 0.2.17 for the
