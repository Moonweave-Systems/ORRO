...
to witnessd. The post-release target state is: `orro` 0.2.22 is published on PyPI.
It becomes true only after `v0.2.22` is tagged and the Trusted-Publishing workflow
completes; until then, PyPI contains the ORRO product line through 0.2.15. This
repository is the canonical 0.2.22 source, whose metadata declares
...
does not assert that the 0.2.22 source candidate is already published.
...
3. Published package: current phase. The ORRO product line is published through
   0.2.1, and source 0.2.22 is prepared for the tag-triggered Trusted-Publishing
